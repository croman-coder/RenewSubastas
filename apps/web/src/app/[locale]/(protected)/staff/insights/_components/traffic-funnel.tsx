...
import { buildFunnelSteps, FUNNEL_STAGES, type FunnelStage } from '@/lib/insights/traffic-summary';
 ...
 * to `home`) AND the step-over-step change versus the stage right before
 * it, so a reader sees both "how big" and "what just happened here" without
 ...
 * drop would silently mislabel real traffic shape as a data bug.
 */
      ...
      {total === 0 ? (
        ...
        <ol className="px-5 pb-5 pt-3 space-y-4">
          {steps.map((step, i) => {
            ...
            const barWidthPct = Math.max(0, Math.min(100, step.pctOfFirst ?? 0));
